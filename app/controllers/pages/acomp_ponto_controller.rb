class Pages::AcompPontoController < PagesController
  ...
  if params[:commit].to_s == "Enviar"
    #puts "enviando valores"
     enviarDados(params[:data_ini],params[:data_fim],params[:cod_pessoa])
      request[:cod_pessoa] = params[:cod_pessoa] 
...
def enviarDados(cod_pessoa,data_ini,data_fim)
...
message = {
  ...
  'parameters' => "'COD_PESSOA="+request[:cod_pessoa]+";DATA_INI="+request[:data_ini]+";DATA_FIM="+request[:data_fim]+";'",
}
